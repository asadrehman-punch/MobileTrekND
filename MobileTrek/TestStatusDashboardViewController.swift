...
class TestStatusDashboardViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //        self.tabBarController?.navigationItem.rightBarButtonItem.hidd
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        print("the section title \(sectionTitle) in number of Rows")
        
        
        if (sectionTitle == "Check In History"){
            ...
            if (currentUser.collectionSiteCheckOut && currentUser.collectionSiteCheckIn){
                return 2
            }else if (currentUser.collectionSiteCheckIn){
                return 1
            }else if (currentUser.collectionSiteCheckOut){
                return 1
            }
            
        }else if (sectionTitle == "Sobriety Date"){
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        self.setupTitlesArrayForSections()
        return sectionsCount
    }
    
    
    
    func setupTitlesArrayForSections(){
        //   sectionTitle.removeAll()
        if (currentUser.questAuthorization){
            sectionTitle.append("Lab Authorization")
        }
        if currentUser.globalSobrietyDate && sobrietyDate != nil {
            sectionTitle.append("Sobriety Date")
        }
        
        if (currentUser.collectionSiteCheckOut || currentUser.collectionSiteCheckIn){
            sectionTitle.append("Collection Site Check-In")
        }
        
        if currentUser.globalCheckInHistory {
            sectionTitle.append("Check In History")
        }
        
        if (currentUser.globalFacilityCheckInHistory){
            sectionTitle.append("Collection Site Check-In/Out History")
        }
...
extension TestStatusDashboardViewController : SideMenuDelegate{
    func surveyTapped() {
